...
from src.v1.auth.schemas import (UserResponse, TokensResponse, VerifyTokenResponse, RefreshTokens,
...
async def test_signup_user_success(api_session: AsyncClient, db: AsyncSession):
    ...
    user_in_db = await UserService.get_by_email(db, random_user_email)
...
async def test_signup_user_failed(api_session: AsyncClient, db: AsyncSession, incorrect_data):
    ...
    try:
        await UserService.get_by_email(db, incorrect_data.get("email"))
...
@pytest.mark.parametrize(
        ...
        [
            {
                "email": "some123@example.com",
                "password": "somepass"
            },
            {
                "email": "some@example.com",
                "password": "somepass213123"
            },
        ]
    )
@pytest.mark.asyncio
...
async def test_success_terminate_all_sessions(api_session: AsyncClient, db: AsyncSession):
    ...
    tokens = TokensResponse(**response.json())
    exists_user = await UserService.get_by_email(db, exists_user_data.get("email"))
    ...
    response = await api_session.post(
        "/api/v1/auth/verify", 
        cookies={"access_token": tokens.data.access_token}
    )
    assert response.status_code == HTTPStatus.OK
    assert response.json() == VerifyTokenResponse().model_dump()
